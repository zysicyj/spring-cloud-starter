...
import com.huan.common.aspect.check.CheckParamUtils;
 ...
 */
@Slf4j
public class CommonExcelImportService extends ExcelImportService {

    private final static Logger LOGGER = LoggerFactory.getLogger(ExcelImportService.class);

    private CommonCellValueService cellValueServer;
    ...
    private List<Object> failCollection;

    CommonExcelImportService() {
        successRow = new ArrayList<Row>();
        failRow = new ArrayList<Row>();
        failCollection = new ArrayList();
        cellValueServer = new CommonCellValueService();
        checkParamUtils = new CheckParamUtils();
    }
                            ...
                            if (excelParams.containsKey(titleString) || isMap) {
                                if (excelParams.get(titleString) != null
                                    ...
                                    try {
                                        ...
                                        Integer fixed = Integer.valueOf(titleString.replace("FIXED_", "") + 1);
                                        errorMsg.append("错误在第").append(row.getRowNum() + 1).append("行第").append(fixed).append("列，原因是：").append(ExcelImportEnum.GET_VALUE_ERROR.getMsg());
                                        if (object instanceof IExcelModel) {
                                            ...
                                            model.setErrorMsg((StringUtils.isNoneBlank(model.getErrorMsg())
                                                    ? model.getErrorMsg() + "," : "") + errorMsg.toString());
                                        }
                                        cell = row.createCell(row.getLastCellNum());
                                        cell.setCellValue((StringUtils.isNoneBlank(cell.getStringCellValue())
                                                ? cell.getStringCellValue() + "," : "") + errorMsg.toString());
                                        failCollection.add(row);
                                        return collection;
                                    }
                                }
                            }
    ...
    public boolean verifyingDataValidity(Object object, Row row, ImportParams params,
        ...
        if (params.isNeedVerify()) {
            ...
            try {
                checkParamUtils.validatorAll(object);
            } catch (Exception e) {
                errorMsg = "\n错误在第" + row.getRowNum() + "行\t" + e.getMessage();
            }
